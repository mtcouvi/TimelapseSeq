...
##############################################################
Exp = 'TL3' # 'TL3' mouse_H TL5
args <- commandArgs(trailingOnly = TRUE)
MapMethod <- 'MT_t5MTMMinformed6_withDups' 
reads <- 'MTnorRNA' # 'MTall' All MTnorRNA
path = '/n/groups/churchman/mc348/TimelapseSeq/Hela_2020_09/'  # 
# samples=c('G1', 'G2', 'G3', 'G4', 'G5')
...
write.table(JointFreq, file=paste0(path, 'MMfrequency/',Exp,'_',MapMethod,'_frag_',reads,'_TcountANDTCconv_',samples[i],'.txt'), sep=("\t"), quote=FALSE, col.names=NA)
}
...
pdf(paste0(path, 'MMfrequency/',Exp,'_', MapMethod, '_frag_',reads,'_LengthAndTcountDist.pdf'), width = 4, height = 8)
...
write.table(TcountDT, file=paste0(path, 'MMfrequency/',Exp,'_',MapMethod,'_frag_',reads,'_TcountDistributions.txt'), row.names=FALSE, sep=("\t"), quote=FALSE)
...
pdf(paste0(path, 'MMfrequency/',Exp,'_', MapMethod, '_frag_',reads,'_AveTs_NumReadsWithMM.pdf'), width = 9, height = 10)
