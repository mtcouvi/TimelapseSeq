...
# Use: ./findMismatches_complete_19_06_14.R *.sam

# Load bash modules for later analysis
system("module load gcc/6.2.0")
system("module load bedtools/2.27.1")

library(gtools, lib.loc="~/R-4.0.1/library/")
...
#old_filename = substr(paste0(dat_file), 1, nchar(paste0(dat_file)) - 11)
#new_filename = paste(old_filename, "mismatches.sam", sep="")
new_filename = paste0(path, "/", paste0(dat_file), "_MM.bed") # For running in parallel
...
for (i in 1:m) {
	...
	if (tmp[2] == "X") {
	   ...
	   for (k in 1:as.numeric(tmp[1])) {
		   ...
		   # Store the current p_read and p_gen values as positions of mismatches
	   	   ...
	   	   q_score <- unname(asc(q_score_char))
    ...
    all_covered_quality_scores <- unname(asc(all_covered_quality))-33
...
write.table(out, file = new_filename, quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
